...
export default async function BlogPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      <h1 className="font-title text-4xl md:text-5xl text-inlar-primary mb-8">
        Blog
      </h1>

      {sorted.length === 0 ? (
        <p className="font-body text-inlar-ink/70">
        ...
        <ul className="space-y-6">
          {sorted.map((post) => (
            ...
            >
              <Link
                href={`/blog/${post.slug}`}
                className="font-title text-2xl text-inlar-primary hover:text-inlar-accent transition-colors"
              >
                {post.entry.title}
              </Link>
              {post.entry.description ? (
                <p className="mt-2 font-body text-inlar-ink/80">
                  {post.entry.description}
                </p>
              ) : null}
